...
router.get("/", verifyJWT, async (req, res) => {
  ...
  const eventData = await CalendarModel.find({orgId : req.user.orgId, collegeId : req.user.collegeId, tags : { $in: ["academic","holiday"]}});
  //eventData = await CalendarModel.find({orgId : req.user.orgId, collegeId : req.user.collegeId, tags : "holiday"});
...
async function targetData(req, res, next) {
    let moreData;
    try {
        moreData = await CalendarModel.findOne({orgId : req.user.orgId, collegeId : req.user.collegeId, _id : req.params.id, tags : req.user.tags});
        if (moreData == null) {
